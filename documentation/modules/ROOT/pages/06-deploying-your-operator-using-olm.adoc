...
Also have a look to
`./config/manifests/base/{app_name}.clusterserviceversion.yaml` this file is a template and is where you should make changes to `name`, `description`, `icon`, etc.

For instance edit the file `{app_name}.clusterserviceversion.yaml` and change `spec.description` with the proposed text.
